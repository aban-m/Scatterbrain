...
def read_latest_id(conn, user_id: str):
    resp = conn.execute(
        "SELECT MAX(entry_id) FROM entries WHERE user_id = ?;", (user_id,)
    )
    ans = resp.fetchone()[0]
...
def set_synced(conn, user_id, val):
    conn.execute(
        "UPDATE Users SET pca_synced = ? WHERE user_id = ?;",
        (
            val,
            user_id,
        ),
...
def get_synced(conn, user_id):
    row = conn.execute(
        "SELECT pca_synced FROM Users WHERE user_id = ?;", (user_id,)
    ).fetchone()
    if not row:
        return True
    return row[0]
...
def is_image(conn, user_id, entry_id):
    row = conn.execute(
        "SELECT FROM Entries WHERE user_id = ? AND entity_id = ?;", (user_id, entry_id)
    ).fetchone()
    if row is None:
...
def get_image(conn, user_id: str, entry_id: int) -> str:
    row = conn.execute(
        "SELECT image_mini_base64 FROM entries WHERE user_id = ? AND entry_id = ?;",
        (user_id, entry_id),
    ).fetchone()
    if not row:
        return ""
...
def delete_entry(conn, user_id: str, entry_id: int):
    conn.execute(
        "DELETE FROM entries WHERE entry_id = ? AND user_id = ?;", (entry_id, user_id)
    )
    user = _read_user(conn, user_id)
    _update_user(conn, user_id, user["total_entries"] - 1, False)
    conn.commit()
...
WHERE user_id = ? AND entry_id = ?;""",
        (new_text, marshal_array(new_embedding), user_id, entry_id),
    )
    conn.commit()
...
def update_pcas(conn, user_id: str, pcas: list):
    ...
    conn.executemany(
        "UPDATE entries SET pca = ? WHERE user_id = ? AND entry_id = ?;",
        [
            (marshal_array(pca), user_id, row["entry_id"])
            for pca, row in zip(pcas, current_ids)
        ],
    )
    ...
    try:
        data = [
            ...
            for x in conn.execute(
                f"SELECT {which} FROM Entries WHERE user_id = ?", (user_id,)
            ).fetchall()
        ]
...
def read_user(conn, user_id: str):
    data = conn.execute(
        "SELECT is_image, entry_id, content FROM entries WHERE user_id = ?", (user_id,)
    ).fetchall()
...
def read_entry(conn, user_id: str, entry_id: int):
    data = conn.execute(
        """'SELECT is_image, entry_id, content FROM entrie
WHERE user_id = ? AND entry_id = ?""",
        (user_id, entry_id),
    ).fetchone()
    return data
