...
from server.core import embeddings as embeddings_utils
from server.db.base import connect
from server.db import ops
from server.websource import reddit
...
def echo():
    ...
    if request.method == "GET":
        ...
        return {"echo": request.json["text"].upper()}
...
def pca():
    if request.method == "PATCH":
        ...
        return {
            "pca": list(zip(*pcas))
        }  # the tuples are converted to lists automatically
    ...
    if request.method == "GET":
        if eid is None:
            ...
            for entry in entries:
                entry["url"] = (
                    url_for("api.image", uid=session["user_id"], eid=entry["entry_id"])
                    if entry["is_image"]
                    else ""
                )
        ...
        new_embedding = embeddings_utils.embed(new_text)
        ...
        if is_text:
            type = request.json.get("type", "direct")
            if type == "direct":
                text: str = request.json["text"]
                embedding = embeddings_utils.embed(text)
                ops.create_text_entry(g.db, session["user_id"], text, embedding)
            elif type == "reddit":
                url = request.json["text"]
                texts = reddit.ingest(url, limit=20)
                dembeddings = embeddings_utils.multi_embed(texts)
                for text, embedding in zip(texts, dembeddings):
                    ops.create_text_entry(
                        g.db, session["user_id"], text, embedding, commit=False
                    )
                g.db.commit()
            else:
                raise ValueError("Unexpected text-entry type.")
        else:
            image_input = request.json["image"]
            is_url = request.json["is_url"]
            result = images.process(image_input, is_url)
            embedding = embeddings_utils.embed(result["description"])
            ops.create_image_entry(
                g.db,
                session["user_id"],
                mini_b64=result["mini_b64"],
