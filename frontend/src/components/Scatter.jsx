...
export default function Scatter() {
  ...
  const { hoveredId, setHoveredId } = useWaiting();
  ...
  useEffect(() => {
    ...
    const colorArray = [...Array(estate.entries.length).keys()].map((i) =>
      i + 1 === hoveredId ? "#a3d9a5" : "blue"
    );
    const sizeArray = [...Array(estate.entries.length).keys()].map((i) =>
      i + 1 === hoveredId ? 20 : 10
    );
